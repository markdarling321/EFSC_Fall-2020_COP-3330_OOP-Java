		...
		for (int maxWaitTime = 1; maxWaitTime <= 4; maxWaitTime++)
		{
			...
			try
			{
				...
				for (int count = threadTime / MAIN_WAIT; count != 0 && msgLoopThread.isAlive(); count--)
				{
					...
					// INTERRUPT msgLoopThread THREAD IF IT IS STILL RUNNING AND INNER for(){} LOOP
					// HAS REACHED ITS FINAL ITERATION
					if (msgLoopThread.isAlive() && count == 1)
					{
						...
						// INTERRUPT msgLoopThread THREAD AND YIELD CONTROL TO msgLoopThread
						// SO ITS catch(){} BLOCK CAN FINISH EXECUTING BEFORE main() THREAD CONTINUES
